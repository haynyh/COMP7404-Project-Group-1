...
task_list = ['bace_split/', 'HIV_split/', 'sider_split/', 'tox21_split/', 'application_split/', 'PDBbind']

def set_task(args):
    if args.dataset in ['bace_split/', 'HIV_split/', 'sider_split/', 'tox21_split/', 'application_split/']:
        args.input_dim = 2048
        args.output_dim = 2
...
def get_train_np_loader(dir_name, batch_size, sub_task=1, num_workers=1, pin_memory=False, dim=2048, fit=False, return_np=False):
    if dir_name in ['bace_split/', 'HIV_split/', 'sider_split/', 'tox21_split/', 'application_split/']:
...
def get_data_loaders(dir_name, batch_size, sub_task=1, num_workers=0, pin_memory=False, dim=2048, fit=False, return_np=False, train_shuffle=True):
    if dir_name in ['bace_split/', 'HIV_split/', 'sider_split/', 'tox21_split/', 'application_split/']:
        dir_name = 'data/' + dir_name
        train_x, train_y = load_chem_data(dir_name + 'train.fgp{}.csv'.format(dim), sub_task)
    ...
    valid_data = Feat(valid_x, valid_y)
    test_data  = Feat(test_x , test_y)
...
####Self-made
def get_pred_data_loaders(dir_name, batch_size, sub_task=1, num_workers=0, pin_memory=False, dim=2048, fit=False, return_np=False, train_shuffle=True):
    if dir_name in ['bace_split/', 'HIV_split/', 'sider_split/', 'tox21_split/', 'application_split/']:
        dir_name = 'prediction/' + dir_name
        #train_x, train_y = load_chem_data(dir_name + 'train.fgp{}.csv'.format(dim), sub_task)
        #valid_x, valid_y = load_chem_data(dir_name + 'valid.fgp{}.csv'.format(dim), sub_task)
        #test_x, test_y = load_chem_data(dir_name + 'test.fgp{}.csv'.format(dim), sub_task)
        pred_x, pred_y = load_chem_data(dir_name + 'pred.fgp{}.csv'.format(dim), sub_task)
    #elif dir_name == 'PDBbind':
    #    train_x, train_y, valid_x, valid_y, test_x, test_y = load_pklgz('data/PDBbind.pkl.gz')
    print('pred data shapes (x.shape, y.shape)')    
    #print(train_x.shape, train_y.shape)
    #print(valid_x.shape, valid_y.shape) 
    print(pred_x.shape, pred_y.shape)

    #if len(set(pred_y)) < 50:
    #    for y in set(pred_y):
    #        print('pred = {}, ACC = {:.4f}'.format(y,len(np.where(pred_y == y)[0]) / len(pred_y)))
    #else:
    #    print('train RMSE = {:.4f}'.format(np.sqrt(np.mean((train_y - np.mean(train_y)) ** 2 ))))
    #    print('test  RMSE = {:.4f}'.format(np.sqrt(np.mean((test_y - np.mean(train_y)) ** 2 ))))
    ...
    pred_loader = DataLoader(
        pred_data, batch_size=batch_size, shuffle=train_shuffle,
        num_workers=num_workers, pin_memory=pin_memory,
    )
